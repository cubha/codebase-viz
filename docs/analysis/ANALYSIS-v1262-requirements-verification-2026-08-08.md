...
| 5 | FE Tab3 `ep_*` 노드 클릭 무반응 | 🟡 | `fe/tab3-api.ts:19,106` | ⚠️ **정정(2026-08-10)**: 아래 서술은 **렌더러가 emit한 mermaid 텍스트만 놓고 관측**한 것이다 — webview 실물 재현에서는 `viewer.html renderDbView()`가 `DIAGRAMS.d`를 항상 erDiagram 파서(`parseDbData`)로 재합성해 `graph LR`(FE Tab3) 입력은 tables 0 → `(No data)`로 통째로 사라진다(**D0**, 신규 항목·P0, v1.2.63 계획 참조). 즉 "라우트 박스는 딥링크되고 `ep_*`만 무반응"이라는 아래 서술은 webview 기준으로 틀렸다 — 라우트 박스조차 렌더되지 않는다. 원 서술은 D0 해소 후에도 유효한 잔여 결함(D5)으로 보존한다: react-router FE의 Tab3는 `graph LR`(flowchart)이라 `.node`가 존재해 클릭 리스너가 붙지만, endpoint 노드는 `graph.nodes` 미등록 합성 노드라 매핑 불가 → 클릭해도 반응 없음. 실측(렌더러 텍스트 기준): partner-mock Tab3 16개 중 9개(`ep_*`). **교훈**: 산출물 텍스트 검증은 webview 렌더 경로를 대체하지 못한다 — 이후 웹뷰 결함 검증은 `scripts/render-harness.mjs` + Playwright 스크린샷을 완료조건으로 삼는다. |

...
## ✨ What's new in v1.2.65

### Click-to-source now lands on the right file in React Router projects

- **Routes generated by `routes.map(...)` used to jump to the router's `map()` line.** Every route in the loop shared that one location instead of pointing at the page it renders — so clicking any of them took you to the same spot. Clicking a route now opens that route's page file.
- **Component nodes carried the router's location instead of their own.** A component living in `src/pages/Code.tsx` deep-linked into `src/router.tsx`. Fixed everywhere the React Router parser creates components, including nested child components, which pointed at the parent that imported them.
- **Routes that can't be traced to a page** now point at their own entry in the route array (e.g. `appRoutes.ts:10`) rather than collapsing onto the `map()` line.
- Upgrading re-runs the analysis automatically — the old cache would otherwise keep replaying the previous, incorrect jump targets.
